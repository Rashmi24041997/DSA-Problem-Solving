...
public static class RecursionEasy
{
    public static void ReverseStack(Stack<int> st1)
    {
    ...
    private static void InsertAtBottom(Stack<int> st1, int top)
    {
        if (st1.Count == 0)
        {
            st1.Push(top);
        ...
        int item = st1.Pop();
        InsertAtBottom(st1, item);
        st1.Push(item);
    }

    public static void SortStack(Stack<int> st1)
    {
        if (st1.Count == 0)
            return;
        int top = st1.Pop();
        SortStack(st1);
        SortStackHelper(st1, top);
    }

    private static void SortStackHelper(Stack<int> st1, int top)
    {
        if (st1.Count == 0 || st1.Peek() < top)
        {
            st1.Push(top);
            return;
        }
    ...
    public static ListNode RemoveElements(ListNode head, int val)
    {
        ...
        dummy.next = head;
        ListNode pre = dummy, crnt = head;
        RemoveElementsHelper(pre, crnt, val);
        //while (crnt != null)
        //{
        //    if (crnt.val == val)
        //        pre.next = crnt.next;
    ...
    private static void RemoveElementsHelper(ListNode pre, ListNode crnt, int val)
    {
        if (crnt == null)
            return;
        if (crnt.val == val)
...
public class RecursionMedium
{
    ...
    */
    ...
    public static double myPowRevWithMod(int x, int n)
    {
        int mod = 1000000007;
        long ans = 1;
        x %= mod;
    ...
    {
        int mod = 1000000007;
        var primes = myPowRevWithMod(4, (int)n / 2) % (mod);
        dynamic evens = 0;
        if (n % 2 == 0)
            evens = myPowRevWithMod(5, (int)n / 2) % (mod);
        else
            evens = myPowRevWithMod(5, (int)(n + 1) / 2) % (mod);

        var prod = (evens * primes) % (mod);
        return (int)prod;
    }
}
